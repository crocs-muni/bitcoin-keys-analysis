...
def demo():
    ...
    print("    - ", PURPLE, "BitcoinPublicKeyParser.process_transaction(", NO_COLOR, "self, txid: ", YELLOW, "str", PURPLE, ")", NO_COLOR, "\n", sep = '')
 ...
 depending on the type of the public key and amount of corresponding signatures.")
    ...
    print("\n   - ", PURPLE, "BitcoinPublicKeyParser.process_block(", NO_COLOR, "self, n:", YELLOW, " int", PURPLE, ")", NO_COLOR, "\n", sep = '')
    ...
    print("\n   - ", PURPLE, "BitcoinPublicKeyParser.process_blocks(", NO_COLOR, "self, start: ", YELLOW, "int", NO_COLOR, ", end: ", YELLOW, "int", PURPLE, ")", NO_COLOR, "\n", sep = '')
    print("     This function calls <BitcoinPublicKeyParser.process_block()> for all blocks in <range(start, end)>, flushes the collected data to .json and .txt files (to <gathered-data> subdirectory, which you'll have to create) with <BitcoinPublicKeyParser.flush_if_needed()> function at the very end of itself and every 10000 keys not ot exhaust RAM, and prints out parsing statistics.")
